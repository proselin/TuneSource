...
<?php if(empty($_SESSION['username'])){?>
	<button class="disable btn btn-outline-secondary btn-block"> Please login for add product to cart</button>
<?php 
}else{ ?>
<div>
			...
			?>
			     ...
			     <ul class="list-group">
			      ...
			      if( mysqli_num_rows($rs) > 0 ){
			        while( $row = mysqli_fetch_assoc($rs) ){
			      ?>
			        <li class="list-group-item"><a href="simple_genre.php?id_gen=<?= $row['id_gen'] ?>"> <?php echo $row['namegenre'] ; ?></a></li>
