...
include('conn.php');
  if(!empty($_GET['edit']) && $_SERVER['REQUEST_METHOD'] == 'POST'){
      ...
      $sql = "UPDATE genre SET namegenre= ?, description= ? where id_gen = {$id}";
...
$target_file = $target_dir . basename($_FILES["fileimage"]["name"]);
  if (move_uploaded_file($_FILES["fileimage"]["tmp_name"], $target_file)) {
    echo "The file ". htmlspecialchars( basename( $_FILES["fileimage"]["name"])). " has been uploaded.<br>";
       $ab = mysqli_query($conn, "UPDATE genre SET image = '$a ' where id_gen = {$id}");
  } else {
...
<body>
  ...
  <div class="form-group col-md-6">
    ...
    <input type="text" name="namegenre" class="form-control" id="ei1" placeholder="Name" value="<?= $row['namegenre'] ?>" required="required">
  </div>
   <div class="form-group col-md-6">
...
<div class="col-md-4">
    ...
    <div class="col-md-4">
      <a href="listgenre.php" class="btn btn-danger">Cancel</a>
    </div>                    
  </div>  
  </form>
</div>
